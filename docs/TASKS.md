...
- [x] Test pull-to-refresh — **confirmed on device 2026-07-24**: a pull re-fetched from the backend and the list updated (folder counts changed to match a live DB edit, then reverted)
...
- [x] **Items blocked by the AI quota get reprocessed automatically** (2026-07-24). Found by the builder asking the right question: *"once the AI limit resets, will these reorganize themselves or stay stuck in Other?"* The answer was stay stuck — `ai_status` was recorded but nothing ever read it back, so `AI_FEATURE_SPEC` 9.3's "a future job can re-process them" had never been built. Worse, the duplicate check added earlier the same day had just closed the one accidental workaround (re-sharing used to create a fresh row that processed normally). Now: `services/reprocess.py` sweeps `quota_exceeded` items using their stored `raw_content`, triggered at startup and after `GET /items`; and re-sharing any non-`ok` item re-runs the AI in place. Verified — a queued item moved Other → Tech & Coding with `created_at`/`url` untouched, the queue drained, the 10-minute rate limit held, and a re-share reprocessed in place without creating a duplicate row. **Mobile side (2026-07-24):** the home screen now shows quota-blocked items in a separate "Processing" group at the top (in the All view only), kept out of the folder chips and their counts so a placeholder item can't inflate "Other". Pending items render with a dedicated `_PendingCard` showing the link host + "Waiting for AI…" rather than an `ItemCard` full of `"Untitled saved item"` placeholder text. `flutter analyze` clean, debug APK builds. **Verified on the physical Infinix 2026-07-24:** with one item temporarily set to `quota_exceeded`, the "Processing (1)" group rendered at the top with the hourglass header, the wrapping subtitle, and a `_PendingCard` showing `example.com` + "Waiting for AI…"; the thick divider separated it from the library; and the "All" count dropped from 23 to 22, proving pending items are excluded from the folder counts. The injected item was restored to its original state afterward. Same session also confirmed the TikTok oEmbed fix on device (a TikTok item showed a real thumbnail and a "Tech & Coding" classification) and that the reprocess queue drained on its own after the 2 PM quota reset
